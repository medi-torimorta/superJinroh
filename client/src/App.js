...
function AbilityList({ abilityIds, abilityCatalog, sourceType, sourceId, itemCardId = null, remainingUseCount, availableAbilityKeys, reservedAbilityKeys, showReservedOverlay = true, onAbilityClick, }) {
    ...
    return (_jsx("div", { className: "ability-list", children: abilityIds.map((abilityId) => {
            ...
            return (_jsxs("div", { className: `ability-entry${showAvailableBorder ? ' is-available' : ''}${isReserved ? ' is-reserved' : ''}${isExhausted ? ' is-exhausted' : ''}${isAvailable && onAbilityClick ? ' is-clickable' : ''}`, onClick: isAvailable && onAbilityClick ? () => onAbilityClick(ability.abilityId, itemCardId) : undefined, role: isAvailable && onAbilityClick ? 'button' : undefined, tabIndex: isAvailable && onAbilityClick ? 0 : undefined, children: [_jsxs("div", { className: "title-with-badge ability-line", children: [_jsx("strong", { children: sourceType === 'ITEM' ? '効果' : ability.displayName }), sourceType !== 'ROLE' ? _jsx(AbilityTypeBadge, { abilityType: ability.abilityType }) : null] }), sourceType === 'HOBBY' && remainingUseCount !== undefined ? _jsxs("div", { className: "ability-remaining-count", children: ["\u6B8B\u308A\u56DE\u6570: ", remainingUseCount ?? '-'] }) : null, _jsx("span", { className: "ability-description-inline", children: ability.description }), isReserved && showReservedOverlay ? (_jsx("div", { className: "reservation-overlay", "aria-hidden": "true", children: _jsx("span", { className: "reservation-pill", children: "\u5BA3\u8A00\u4E88\u7D04\u4E2D" }) })) : null, isExhausted ? _jsx("div", { className: "ability-disabled-overlay", "aria-hidden": "true" }) : null] }, ability.abilityId));
        }) }));
}
...
function MasterInfoBlock({ title, displayName, description, imagePath, faction, hobbyType, abilityIds, abilityCatalog, sourceType, sourceId, remainingUseCount, availableAbilityKeys, reservedAbilityKeys, onAbilityClick, collapsed, onToggle, collapsedTitle, collapsedTitleAddon, titleHoverLabel, titleHoverContent, }) {
